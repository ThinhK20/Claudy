...
- [x] **Step 3: Verify** — `npx tsc --noEmit` clean. Manual: switch System→Dark→Light in the running app; `.dark` class toggles on `<html>` immediately; restart persists the choice.
- [x] **Step 4: Commit** — `feat: add theme picker to settings appearance card`
...
- [x] **Step 4: Verify** — `cd src-tauri; cargo test` all green (13 existing shortcuts tests unaffected).
- [x] **Step 5: Commit** — `feat: add suspend and resume commands for global shortcuts`
...
- [x] **Step 3: Verify** — `npx tsc --noEmit` clean. Manual E2E in the running app (see end-of-phase Verification for the full checklist): while capturing, pressing the LIVE dictation combo is captured instead of toggling dictation; saving a NEW dictation combo succeeds (proves resume-before-onChange ordering).
- [x] **Step 4: Commit** — `feat: suspend global shortcuts while recording a combo`
...
- [x] **Step 1: Write the failing tests** — new `#[cfg(test)] mod tests` at the bottom of `inject.rs`:
...
- [x] **Step 3:** Update call sites: `inject.rs` `insert_text` → `send_chord_key('v')`; `selection.rs` `read` → `crate::inject::send_chord_key('c')`.
- [x] **Step 4: Verify** — `cd src-tauri; cargo test` all green (3 new tests pass, 2 existing selection tests unaffected). Manual smoke: dictation still pastes; prompt auto-paste/copy still works.
- [x] **Step 5: Commit** — `fix: use Cmd as the synthetic chord modifier on macOS`
...
- [x] **Step 3: Verify** — `npm run tauri build` produces the NSIS installer; install it, launch from the Start Menu shortcut, dictation shortcut works, then uninstall cleanly.
- [x] **Step 4: Commit** — `feat: configure NSIS-only installer and document platform builds`
